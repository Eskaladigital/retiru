/**
 * Descripción de ficha: gpt-5.6-terra + web_search nativo (Responses API).
 * Alineado con src/lib/openai/center-description.ts. Sin SerpAPI.
 */
...
/**
 * @param {string} apiKey
 * @param {{ name: string, city: string, province: string, address?: string|null, type?: string|null, website?: string|null, services_es?: string[]|null }} center
 */
export async function generateCenterDescriptionEs(apiKey, center) {
  const ficha = [
    `Nombre: ${center.name}`,
    `Ciudad: ${center.city}`,
