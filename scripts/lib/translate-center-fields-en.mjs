/**
 * SYNC: mismo contrato JSON y reglas que src/lib/openai/translate-center-en.ts
 */
...
/**
 * @param {string} apiKey
 * @param {{ descriptionEs: string, servicesEs: string[], scheduleSummaryEs: string | null, priceRangeEs: string | null }} input
 */
export async function translateCenterFieldsToEn(apiKey, input) {
  const payload = {
  ...
  if (services_en.length !== input.servicesEs.length && input.servicesEs.length > 0) {
    services_en = input.servicesEs.map((es, i) => services_en[i] || es);
  }

  const schedule_summary_en =
    parsed.schedule_summary_en == null || String(parsed.schedule_summary_en).trim() === ''
      ? null
      : String(parsed.schedule_summary_en).trim();

  const price_range_en =
    parsed.price_range_en == null || String(parsed.price_range_en).trim() === ''
      ? null
      : String(parsed.price_range_en).trim();
