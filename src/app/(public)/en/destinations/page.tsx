// /en/destinations — Destinations list (Supabase)
...
export default async function DestinationsPageEN() {
  ...
  return (
    <div className="container-wide py-12">
      <h1 className="font-serif text-[clamp(28px,4vw,40px)] text-foreground mb-2">Destinations</h1>
      <p className="text-[#7a6b5d] mb-10 max-w-lg">The most special corners of Spain for your next retreat</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        {destinations.map((d) => {
          ...
          const name = d.name_en || d.name_es;
          const desc = d.description_en || d.description_es || '';
          return (
            <Link key={d.id} href={`/en/destinations/${d.slug}`} className="group rounded-2xl overflow-hidden relative cursor-pointer hover:-translate-y-1 transition-transform duration-300">
              <div className="aspect-[4/3] relative">
                <ImageWithFallback src={img} alt={name} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.06]" />
