...
const chipDateFmt = new Intl.DateTimeFormat('en-GB', { weekday: 'short', day: 'numeric', month: 'short' });
...
function durationLabel(days: number, hours?: number | null) {
  if (days <= 1) return hours ? `${hours} ${hours === 1 ? 'hour' : 'hours'}` : '1 day';
...
export default async function RetreatDetailPageEN({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Holds that occupy a spot but are not in confirmed_bookings
        ...
        <div className="flex gap-10">
          <div className="flex-1 max-w-3xl">
            <div className="mb-6">
              ...
              {/* Recurring event: upcoming dates */}
              {seriesDates.length > 1 && (
                <div className="mt-4">
                  <p className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-[#7a6b5d]">
                    <Repeat size={13} /> Recurring event · upcoming dates
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {seriesDates.map((d) => (
                      d.slug === r.slug ? (
                        ...
                        <Link key={d.slug} href={`/en/retreat/${d.slug}`} className="badge-sand hover:bg-sand-200 transition-colors">
                ...
                <div className="mt-4 rounded-lg bg-sage-50/60 border border-sage-200/60 p-3 text-center">
                    ...
                    {payNow
                      ...
                      : isManualConfirmation
                        ...
                        : !onlinePaymentsEnabled
                          ? 'Registration without payment for now · We\u2019ll email you when online payment is available'
                  ...
                  <p className="text-[11px] text-muted-foreground/70 mt-1.5">
                    {payNow
                      ...
                      : !onlinePaymentsEnabled
                        ? 'No card needed now · The organizer receives your registration'
      ...
      <div className="sticky-cta lg:hidden">
        <div className="flex items-center justify-between gap-3">
          <div className="shrink-0">
            ...
            <p className="text-[10px] text-muted-foreground">per person · <Shield size={10} className="inline" /> {onlinePaymentsEnabled ? 'Secure payment' : 'No charge now'}</p>
          </div>
          <div className="flex items-center gap-2">
