...
function LoginFormEN() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      const supabase = createClient();
      const callbackUrl = `${window.location.origin}/api/auth/callback?redirect=${encodeURIComponent(redirect)}&locale=en`;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white border border-sand-200 rounded-2xl p-8 shadow-soft">
          {searchParams.get('error') === 'auth_failed' && (
            <p className="mb-4 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg">Error signing in with Google. Please try again.</p>
          )}
          {error && (
          ...
          <div className="relative my-6"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-sand-200" /></div><div className="relative flex justify-center"><span className="bg-white px-3 text-xs text-[#a09383]">or continue with</span></div></div>

          <button type="button" onClick={handleGoogleLogin} disabled={loading} className="w-full flex items-center justify-center gap-3 bg-white border border-sand-300 py-3 rounded-xl text-sm font-medium hover:bg-sand-50 transition-colors disabled:opacity-60">
