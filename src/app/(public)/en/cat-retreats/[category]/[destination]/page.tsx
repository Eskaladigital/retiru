...
// Ver nota en `es/retiros-[category]/page.tsx`.
...
export default async function RetreatsCategoryDestinationPage({ params }: { params: Promise<{ category: string; destination: string }> }) {
  ...
  const pastRetreats =
    retreats.length === 0
      ? (await getPastPublishedRetreats({ categorySlug: dbSlug, destinationSlug: destination, limit: 12 })).retreats
      : [];
      ...
      <div className="container-wide py-10">
        ...
        {retreats.length === 0 ? (
          <PastRetreatsFallback
            ...
            heading={`No ${cat.name_en.toLowerCase()} retreats in ${dest.name_en} with open dates`}
