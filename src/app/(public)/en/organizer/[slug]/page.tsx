  ...
  return (
    <div className="container-wide py-12">
      ...
      <div className="flex flex-col md:flex-row gap-6 items-start mb-12">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            {organizer.status === 'verified' && <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-sage-100 text-sage-700">✓ Verified</span>}
          </div>
          <div className="flex flex-wrap items-center gap-4 text-sm text-[#7a6b5d] mb-4">
            {organizer.location && <span>📍 {organizer.location}</span>}
            <span>⭐ {organizer.avg_rating?.toFixed(1) ?? '–'} ({organizer.review_count ?? 0} reviews)</span>
      ...
      <section className="mb-12">
        <h2 className="font-serif text-2xl mb-6">Retreats by {organizer.business_name}</h2>
        {retreatList.length === 0 ? (
          <p className="text-[#7a6b5d]">This organizer has no published retreats yet.</p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {retreatList.map((r: Record<string, unknown>) => {
              ...
              const title = (r.title_en as string) || (r.title_es as string);
              return (
                <Link key={r.id as string} href={`/en/retreat/${r.slug}`} className="group bg-white rounded-2xl overflow-hidden border border-sand-200 transition-all duration-[350ms] hover:shadow-elevated hover:-translate-y-1">
      ...
      {/* Reviews */}
      <section>
        <h2 className="font-serif text-2xl mb-6">Reviews</h2>
        <div className="bg-white border border-sand-200 rounded-2xl p-8 text-center">
          <p className="text-[#7a6b5d]">No reviews yet for this organizer.</p>
        </div>
      </section>
    </div>
  );
