...
import { CheckinClient } from '@/app/(public)/es/(organizer)/panel/eventos/[id]/checkin/CheckinClient';
...
export default async function CheckinPageEn({ params }: Props) {
  const { id } = await params;

  const supabase = await createServerSupabase();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect(`/en/login?redirect=/en/panel/eventos/${id}/checkin`);
  ...
  const attendees: CheckinAttendee[] = (bookings ?? []).map(
    (b: {
      ...
      id: b.id,
      name: b.profiles?.full_name || 'Guest',
      avatar: b.profiles?.avatar_url,
      checkedIn: !!b.checked_in_at,
      time: b.checked_in_at
        ? new Date(b.checked_in_at).toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit' })
        : null,
    }),
  );
  ...
  return (
    <div>
      <Link href={`/en/panel/eventos/${id}`} className="inline-flex items-center gap-1.5 text-sm text-terracotta-600 font-medium mb-6">
        ← Back to retreat
      </Link>
      <h1 className="font-serif text-2xl text-foreground mb-2">Check-in: {retreat.title_es}</h1>
      <p className="text-sm text-[#7a6b5d] mb-6">
        {checkedCount} of {attendees.length} guests checked in
      </p>
      ...
      <div className="bg-white border border-sand-200 rounded-2xl p-6 mb-6 text-center">
        <p className="text-sm text-[#7a6b5d] mb-3">Scan the guest QR or mark manually</p>
