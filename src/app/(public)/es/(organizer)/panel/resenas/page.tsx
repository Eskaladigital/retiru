...
export default async function ResenasPage() {
  ...
  if (!user) redirect('/es/login?redirect=/es/panel/resenas');
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <span className="text-sm text-[#7a6b5d]">{list.length} reseñas</span>
        {unreplied > 0 && (
          <>
            <span className="text-sm text-[#a09383]">·</span>
            <span className="text-sm text-amber-600">{unreplied} sin responder</span>
          </>
        )}
      </div>
