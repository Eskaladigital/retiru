...
export default async function CentroDetailPage({ params }: Props) {
  ...
  return (
    <div className="container-wide py-12">
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-[#7a6b5d] mb-6">
            {C.type && <Link href={`/es/centros/${CENTER_TYPE_URL_ES[C.type] || C.type}`} className="text-[11px] font-semibold px-2.5 py-1 rounded-full bg-sage-100 text-sage-700 hover:bg-sage-200 transition-colors">{getCenterTypeLabel(C.type)}</Link>}
          ...
          {C.description_es && (
            <div className="mb-8">
              ...
              <RetreatDescriptionBody content={C.description_es} />
            </div>
          )}
          ...
          <div className="bg-white border border-sand-200 rounded-2xl p-6 sticky top-24">
            ...
            <div className="space-y-3">
              ...
              {facebookProfileHref(C.facebook) && (
                <div className="text-sm"><span className="text-[#a09383] block text-xs uppercase tracking-wider font-semibold mb-0.5">Facebook</span><a href={facebookProfileHref(C.facebook)!} target="_blank" rel="noopener noreferrer" className="text-foreground hover:text-terracotta-600 transition-colors break-all">Ver página</a></div>
              )}
            </div>
      ...
      {/* Otros centros del mismo tipo en la misma provincia (enlazado interno programático) */}
      {relatedCenters.length > 0 && C.type && provinceSlug && (
        <section className="mt-14">
          <div className="flex items-end justify-between gap-4 mb-5 flex-wrap">
            <h2 className="font-serif text-2xl text-foreground">
              Otros centros de {getCenterTypeLabel(C.type).toLowerCase()} en {C.province}
            </h2>
            <Link
              href={`/es/centros/${CENTER_TYPE_URL_ES[C.type] || C.type}/${provinceSlug}`}
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: jsonLdScript(jsonLdBreadcrumb([
            ...
            ...(C.type ? [{ name: `Centros de ${getCenterTypeLabel(C.type)}`, url: `/es/centros/${CENTER_TYPE_URL_ES[C.type] || C.type}` }] : []),
