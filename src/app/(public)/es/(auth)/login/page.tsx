...
function LoginForm() {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      const supabase = createClient();
      const callbackUrl = `${window.location.origin}/api/auth/callback?redirect=${encodeURIComponent(redirect)}&locale=es`;
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white border border-sand-200 rounded-2xl p-8 shadow-soft">
          {searchParams.get('error') === 'auth_failed' && (
            <p className="mb-4 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg">Error al iniciar sesión con Google. Inténtalo de nuevo.</p>
          )}
          {error && (
