...
import { isGenericDescription, stripMarkdownForPreview, CENTER_FILTER_OPTIONS_ES, getCenterTypeLabel, VALID_CENTER_TYPE_SLUGS, PUBLIC_DIRECTORY_CENTER_TYPE_SLUGS, getSearchTokens, matchesAllTokens, generateSlug, matchesPlaceSlug } from '@/lib/utils';
    ...
    <div className="container-wide py-10">
      ...
      {hasActiveFilters && !showFilters && (
        <div className="flex flex-wrap gap-2 mb-6">
          ...
          {selectedType !== 'Todos' && <span className="inline-flex items-center gap-1 text-xs bg-sage-100 text-sage-700 px-3 py-1.5 rounded-full font-medium">{getCenterTypeLabel(selectedType)} <button onClick={() => setSelectedType('Todos')}><X size={12} /></button></span>}
                  ...
                  <div className="flex items-start justify-between gap-2 mb-1.5">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        {c.type && <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-sage-100 text-sage-700">{getCenterTypeLabel(c.type)}</span>}
