...
- **Panel organizador y asistentes inscritos sin cobro (fix 2026-08-01):** `/es/panel/asistentes` (`GET /api/organizer/attendees`), check-in (lista + PATCH), broadcast, analíticas y KPI «reservas este mes» incluyen `reserved_no_payment` y `pending_payment`. Cancelar un evento también cancela/notifica esas inscripciones; borrar evento/admin bloquea si hay cualquier inscripción activa. En «Mis eventos» del panel se listan además las ocurrencias de serie con reservas (no solo `is_series_next`). Asistentes agrega por serie (columnas Eventos / Fechas). `getPublishedRetreats` resta holds sin cobro de `available_spots` en listados. Email de nueva reserva → `/panel/asistentes`.
