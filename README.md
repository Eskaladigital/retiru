...
54. `supabase/migrations/054_bookings_unique_active_attendee_retreat.sql` — índice único parcial: una sola reserva activa por asistente y ocurrencia (evita doble inscripción por race).
...
- **Panel organizador y asistentes inscritos sin cobro (fix 2026-08-01):** `/es/panel/asistentes`, check-in, broadcast, analíticas y KPIs incluyen `reserved_no_payment`/`pending_payment`. Cancelar notifica esas inscripciones; borrar bloquea con cualquier inscripción activa. Capacidad de checkout/series/fichas/geo/`getPublishedRetreats` resta todos los holds (`HOLD_ENROLLMENT_STATUSES`). Crons de recordatorio y reviews incluyen inscritos sin cobro. Índice único parcial `054` evita doble inscripción por race (aplicar en prod: `npm run db:apply-bookings-unique-active` o SQL Editor). Mis reservas distingue lanzamiento vs mínimo; fechas programadas enlazan a reservas.
