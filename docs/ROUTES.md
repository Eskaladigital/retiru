...
| `/es/retiros-retiru` | `src/app/(public)/es/destino-retiros/page.tsx` (vía rewrite del middleware) | Lista experiencias (`getPublishedRetreats`: publicados, `start_date ≥ hoy+2`). **Ventana pública** (`src/lib/series.ts`): hoy y mañana NO salen (`SERIES_PUBLIC_LEAD_DAYS = 2`); una serie se **colapsa a una tarjeta** (`collapseSeriesToOne`) y lleva las **4 próximas fechas** (`SERIES_PUBLIC_OPEN_DATES`) como chips (`SeriesDateChips`) cuando es diaria. Ya no se filtra por `is_series_next`. Filtro UX `?formato=clases` (`duration_days = 1`) / `?formato=retiros` (`duration_days > 1`); EN: `?format=classes` / `?format=retreats` |
