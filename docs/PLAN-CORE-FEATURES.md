...
- Selector de archivos (múltiple), preview, eliminar, elegir portada; **generar portada con IA** (gpt-image-2) opcional; si no hay fotos, al guardar puede generarse portada automática (OpenAI en servidor). Desde abril de 2026 se prioriza explícitamente **fotografía hiperrealista** y se abandona el look ilustrado de generaciones antiguas.
- Subida al bucket `retreat-images` desde el cliente (RLS, carpeta `retreats/`); persistencia en `retreat_images` con `sort_order` en `POST /api/retreats/create` y `PATCH /api/retreats/[id]`. Ruta API `POST /api/storage/retreat-images` queda como alternativa legacy.
